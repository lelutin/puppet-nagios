...
use vars qw($opt_H $opt_N $opt_r $opt_R $opt_T $opt_d $opt_h $opt_i $opt_v
$opt_V) ;
...
kill 13, $pid ;
print qq(Failed. Max hops ($max_hops)
exceeeded: incomplete after $hops hops, "$route".\n) ;
...
kill 13, $pid ;
print qq(Failed. Last 2 routers ($last_2_is)
repeated, "$route".\n) ;
exit $ERRORS{CRITICAL} ;
}
...
print qq(Ok. Traceroute to "$TargetHost" via expected route "$route"
($rta_list).\n) ;
exit $ERRORS{OK};
} else {
...
return $prev_route =~ /$current_route/;
}

sub print_usage () {
print "Usage: $PROGNAME [ -R <route_string>|-r ] [ -d -T timeout -v -h
-i ] -H <host>\n";
}
...
that the route string option (-R) matches the list of routers
...
Max number of hops.
-r
Record current route (and output to STDOUT). Useful for getting the value of
-R option ...
-v
Greater verbosity.
-R
Mandatory route string ie r1-r2-... where ri is the ip address of the ith
router.
-T
Maximum time (seconds) to wait for the traceroute command to complete.
Defaults to $TIMEOUT seconds.
